...
public class AppRedissonConfig {
    ...
    public RedissonClient redisson() {
        ...
        int timeout = redisProperties.getTimeout() != null ? (int) redisProperties.getTimeout()
                                                                                  .toMillis() : 10000;
        ...
        if (redisProperties.getSentinel() != null) {
            ...
            config.useSentinelServers()
                  // 设置主节点名称
                  .setMasterName(redisProperties.getSentinel()
                                                .getMaster())
                  // 添加哨兵节点地址
                  .addSentinelAddress(convert(redisProperties.getSentinel()
                                                             .getNodes()))
                  // 设置数据库编号
            ...
            config.useClusterServers()
                  // 添加集群节点地址
                  .addNodeAddress(convert(redisProperties.getCluster()
                                                         .getNodes()))
                  // 设置连接超时时间
                  .setConnectTimeout(timeout)
                  // 设置密码
    ...
    private String[] convert(List<String> nodesObject) {
        ...
        return nodesObject.stream()
                          ...
                          .map(node -> node.startsWith(REDIS_PROTOCOL_PREFIX) || node.startsWith(
